...
public class Buoyancy : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        // Check to make sure we're inside our water bounds
        if (transform.position.x <= 3.85f && transform.position.z >= 192.48f && transform.position.x >= -110.2f && transform.position.z <= 367.5)
        {
